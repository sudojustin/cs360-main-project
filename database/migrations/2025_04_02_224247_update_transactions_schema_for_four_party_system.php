...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // Create new transactions table with the correct columns
    ...
    public function down(): void
    {
        ...
        // Create the original transactions table structure 
        Schema::create('transactions', function (Blueprint $table) {
            $table->id('transaction_id');
            $table->foreignId('initiator_id')->constrained('users')->onDelete('cascade');
            $table->foreignId('counterparty_id')->constrained('users')->onDelete('cascade');
            $table->foreignId('partner_initiator_id')->nullable()->constrained('users')->onDelete('cascade');
            $table->foreignId('partner_counterparty_id')->nullable()->constrained('users')->onDelete('cascade');
            $table->foreignId('productp_id')->constrained('products')->onDelete('cascade');
            $table->integer('quantity_p')->default(1);
            $table->foreignId('producte_id')->constrained('products')->onDelete('cascade');
            $table->integer('quantity_e')->default(1);
