                    ...
                    <div class="relative px-6 py-5 sm:px-8 sm:py-6">
                        <div class="flex flex-col md:flex-row items-center justify-between">
                            <div class="mb-4 md:mb-0 md:mr-8">
                                ...
                                <p class="text-green-100 text-base max-w-2xl">Manage your account settings and trading partner information.</p>
                            </div>
                            
                            <div class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4">
                                <div class="bg-white bg-opacity-20 rounded-lg p-3 text-center">
                                    <div class="text-xl font-bold text-white">{{ Auth::user()->name }}</div>
                                    <div class="text-green-100 text-xs">Your Name</div>
                                </div>
                                <div class="bg-white bg-opacity-20 rounded-lg p-3 text-center">
                                    <div class="text-xl font-bold text-white">{{ Auth::user()->partner ? Auth::user()->partner->name : 'Not Set' }}</div>
                                    <div class="text-green-100 text-xs">Trading Partner</div>
                                </div>
                                <div class="bg-white bg-opacity-20 rounded-lg p-3 text-center">
                                    <div class="text-xl font-bold text-white">{{ Auth::user()->created_at->format('M Y') }}</div>
                                    <div class="text-green-100 text-xs">Member Since</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 flex flex-wrap gap-3">
                            ...
                            <a href="{{ route('offers') }}" class="inline-flex items-center px-3 py-1.5 bg-green-800 text-white rounded-md hover:bg-green-700 transition-colors duration-200 shadow-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
